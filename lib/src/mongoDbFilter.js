...
function isLeafTypeTypeGuard(fieldFilter, fieldType) {
    return graphql_1.isLeafType(fieldType);
}
...
function parseMongoDbScalarFilterOpr(opr, graphQLFilter) {
    var _a, _b;
    ...
    if (["$in", "$nin", "$all"].includes(opr)) {
        if (graphQLFilter['values']) {
            return _a = {}, _a[opr] = graphQLFilter['values'], _a;
        }
    }
    else if (graphQLFilter['value'] !== undefined) {
        return _b = {}, _b[opr] = graphQLFilter['value'], _b;
    }
    return {};
}
/////////////////////////////////////////////////////////////////////
